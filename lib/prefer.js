 ...
 * RFC 9110 `token` and `quoted-string` grammars. Preference names are
 * normalized to lowercase per RFC 7240 §2 (case-insensitive comparison).
 * Preference values are also normalized to lowercase as a Postel's Law
 * leniency — RFC 7240 §2 specifies values as case-sensitive, but all
 * IANA-registered preference values are lowercase tokens and practical
 * interoperability benefits from normalization.
 *
 * Supports:
 * - Simple tokens (`respond-async` → `{'respond-async': true}`)
 * - Token=value pairs (`return=minimal` → `{return: 'minimal'}`)
 * - Mixed-case normalization (`Return=Minimal` → `{return: 'minimal'}`)
  ...
  while (i < str.length) {
    ...
    if (ch === 0x22) {
      ...
      while (j < str.length) {
        ...
        if (qch === 0x2c && firstComma === -1) firstComma = j;
        if (qch === 0x22) {
          closed = true;
          j++;
          break;
      ...
      if (!closed) return firstComma === -1 ? str.length : firstComma;
      i = j;
      continue;
    }
    i++;
...
export default function parsePrefer(header) {
  ...
  while (pos < len) {
    ...
    const name = header.slice(pos, nameEnd).toLowerCase();
    ...
    if (pos < len && header.charCodeAt(pos) === 0x22) {
      ...
      if (qs.value === undefined) {
        pos = skipToNextComma(header, qs.end);
        if (pos < len) pos++;
        continue;
      }
      value = qs.value.toLowerCase();
      pos = qs.end;
    } else {
      ...
      value = header.slice(pos, valueEnd).toLowerCase();
      pos = valueEnd;
    }
