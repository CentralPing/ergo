...
function parseQuotedString(str, start) {
  ...
  while (i < str.length) {
    ...
    if (ch === 0x5c && i + 1 < str.length) {
      ...
      // quoted-pair target: HTAB / SP / VCHAR (%x21-7E) / obs-text (%x80+)
      if (next === 0x09 || (next >= 0x20 && next !== 0x7f)) {
        if (valid) {
    ...
    // qdtext: HTAB / SP / %x21 / %x23-5B / %x5D-7E / obs-text (%x80+)
    if (ch === 0x09 || (ch >= 0x20 && ch <= 0x7e && ch !== 0x22 && ch !== 0x5c) || ch >= 0x80) {
      i++;
      continue;
    }
