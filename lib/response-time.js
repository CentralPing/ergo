 ...
 * `ergo-router/lib/auto-wrap.js` (router mode). Both resolve the `timing` option via
 * {@link resolveTimingConfig} before calling {@link applyResponseTiming}.
 *
...
/**
 * Resolve the `timing` option into effective header/precision, or undefined when disabled.
 *
 * @param {boolean|object} [timing] - `true` for defaults, or `{header?, precision?}`
 * @returns {{header: string, precision: number}|undefined}
 */
export function resolveTimingConfig(timing) {
  if (!timing) return undefined;
  if (typeof timing === 'object') {
    return {
      header: timing.header ?? DEFAULT_TIMING_HEADER,
      precision: timing.precision ?? DEFAULT_TIMING_PRECISION
    };
  }
  return {
