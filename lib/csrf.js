 ...
 *
 * @param {string} secret - Shared HMAC secret; throws `TypeError` if not a string
 ...
 */
function issue(secret, uuid = randomUUID(), encoding = 'base64') {
  if (typeof secret !== 'string') {
    throw new TypeError('Missing required parameter: "secret"');
  }
 ...
 * @param {string} key.secret - Shared HMAC secret; throws `TypeError` if not a string
 * @param {string} key.uuid - UUID from the CSRF cookie; throws `TypeError` if not a string
 * @param {string} [key.encoding='base64'] - Encoding used when the token was issued
 * @returns {boolean} - `true` if the token is valid, `false` otherwise
 * @throws {TypeError} If `secret` or `uuid` are not strings
 */
function verify(token, {secret, uuid, encoding = 'base64'} = {}) {
  if (typeof secret !== 'string') {
    throw new TypeError('Missing required parameter: "secret"');
  }
  if (typeof uuid !== 'string') {
    throw new TypeError('Missing required parameter: "uuid"');
  }
