 ...
 * @param {Set<string>} [redactSet] - Header names to replace with '[REDACTED]' in the snapshot
 ...
 */
export default function buildResponseInfo(req, res, startTime, redactSet) {
  const contentLength = res.getHeader('content-length');
  const parsed = contentLength !== undefined ? parseInt(String(contentLength), 10) : undefined;

  return {
    statusCode: res.statusCode,
    headers: redactHeaders(res.getHeaders(), redactSet),
    method: req.method,
    url: req.url,
