...
import createSend from '../http/send.js';
...
describe('[Contract] lib/from-connect', () => {
  ...
  it('Connect middleware that ends response directly results in empty response', async () => {
    ...
    const shortCircuitPipeline = compose(
      fromConnect(shortCircuitMw),
      () => ({statusCode: 200, body: {should: 'not reach'}}),
      createSend()
    );

    const shortCircuitHandler = createHandler(shortCircuitPipeline, createSend());
  ...
  it('Connect middleware error is caught by handler error path', async () => {
    ...
    const errorHandler = createHandler(errorPipeline, createSend());
