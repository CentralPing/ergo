...
describe('[Boundary] lib/authorization', () => {
  ...
  describe('Bearer scheme', () => {
    ...
    it('returns 401 with WWW-Authenticate when rejected with invalid_token', async () => {
      const authorizer = createAuthorizer([
        {
          ...
          authorizer: async () => ({authorized: false, info: {error: 'invalid_token'}})
        }
      ]);
      ...
      assert.equal(typeof result.info.authenticate, 'string');
      assert.ok(result.info.authenticate.includes('error="invalid_token"'));
    });
    ...
    it('returns 400 for invalid_request error type', async () => {
      const authorizer = createAuthorizer([
        {
          type: 'Bearer',
          authorizer: async () => ({authorized: false, info: {error: 'invalid_request'}})
        }
      ]);
    ...
    it('returns 403 for insufficient_scope error type', async () => {
      const authorizer = createAuthorizer([
        {
          type: 'Bearer',
          authorizer: async () => ({authorized: false, info: {error: 'insufficient_scope'}})
        }
      ]);
      ...
      const authorizer = createAuthorizer([
        {
          ...
          authorizer: async () => ({
            ...
            info: {
              ...
              error_uri: 'https://example.com/errors/expired'
            }
          })
        }
      ]);
  ...
  describe('custom ($default) scheme', () => {
    ...
    it('routes strategy type Constructor through $default (not Object)', async () => {
      let received;
      const authorizer = createAuthorizer([
        {
          type: 'Constructor',
          authorizer: async (_attrs, creds) => {
            received = creds;
            return {authorized: true, info: {scheme: 'Constructor'}};
          }
        }
      ]);

      const result = await authorizer('Constructor raw-creds');
      assert.equal(result.authorized, true);
      assert.equal(received, 'raw-creds');
      assert.deepEqual(result.info, {scheme: 'Constructor'});
    });
  });
  ...
  describe('WWW-Authenticate challenge format', () => {
    it('builds correct WWW-Authenticate for multiple strategies', async () => {
      ...
      assert.equal(result.info.statusCode, 401);
      assert.equal(typeof result.info.authenticate, 'string');
      assert.ok(result.info.authenticate.includes('Basic'));
      assert.ok(result.info.authenticate.includes('Bearer'));
  ...
  describe('createDispatcher authenticate attribute sanitization', () => {
    it('escapes double quotes in realm attribute', async () => {
      ...
      const result = await authorizer('Unknown scheme');
      const challenge = result.info.authenticate;
    ...
    const strategies = [
      {
        type: 'Bearer',
        attributes: {realm: 'API'},
        authorizer: async () => ({authorized: true, info: {}})
      }
    ];

    it('returns 401 for Authorization: Constructor (not TypeError)', async () => {
      const authorizer = createAuthorizer(strategies);
      const result = await authorizer('Constructor foo');
      assert.equal(result.authorized, false);
      assert.equal(result.info.statusCode, 401);
    });

    it('returns 401 for Authorization: __proto__ (not TypeError)', async () => {
      const authorizer = createAuthorizer(strategies);
      const result = await authorizer('__proto__ baz');
      assert.equal(result.authorized, false);
      assert.equal(result.info.statusCode, 401);
    });
  });
      ...
      const authorizer = createAuthorizer([
        {
          ...
          authorizer: async () => ({
            authorized: false,
            info: {error: 'invalid_token', error_description: 'token "expired"'}
          })
        }
      ]);
      ...
      const authorizer = createAuthorizer([
        {
          ...
          authorizer: async () => ({
            authorized: false,
            info: {error: 'invalid_token', error_description: 'bad\r\nInjected: header'}
          })
        }
      ]);
      const result = await authorizer('Bearer bad-token');
      ...
      const authorizer = createAuthorizer([
        {
          ...
          authorizer: async () => ({
            authorized: false,
            info: {error: 'invalid_token', error_description: 'path\\to\\resource'}
          })
        }
      ]);
      const result = await authorizer('Bearer bad-token');
