...
describe('[Boundary] lib/link', () => {
  describe('formatLinkHeader', () => {
    ...
    it('throws when href contains ">"', () => {
      assert.throws(() => formatLinkHeader([{href: '/bad>path', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href contains characters not permitted in a URI-reference'
      });
    });
    ...
    it('throws when href contains space', () => {
      assert.throws(() => formatLinkHeader([{href: '/bad path', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href contains characters not permitted in a URI-reference'
      });
    ...
    it('throws when href contains backslash', () => {
      assert.throws(() => formatLinkHeader([{href: '/bad\\path', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href contains characters not permitted in a URI-reference'
      });
    ...
    it('throws when href contains backtick', () => {
      assert.throws(() => formatLinkHeader([{href: '/bad`path', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href contains characters not permitted in a URI-reference'
      });
    ...
    it('throws when href is empty string', () => {
      assert.throws(() => formatLinkHeader([{href: '', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href must be a non-empty URI-reference'
      });
    });

    it('throws when href contains bare percent sign', () => {
      assert.throws(() => formatLinkHeader([{href: '/bad%path', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href contains characters not permitted in a URI-reference'
      });
    ...
    it('throws when href contains truncated percent-encoding', () => {
      assert.throws(() => formatLinkHeader([{href: '/bad%2', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href contains characters not permitted in a URI-reference'
      });
    });

    it('throws when href contains percent followed by non-hex digits', () => {
      assert.throws(() => formatLinkHeader([{href: '/%GGpath', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href contains characters not permitted in a URI-reference'
      });
    });

    it('throws when href ends with bare percent', () => {
      assert.throws(() => formatLinkHeader([{href: '/path%', rel: 'next'}]), {
        name: 'TypeError',
        message: 'Link href contains characters not permitted in a URI-reference'
      });
    });

    it('accepts href with percent-encoded characters', () => {
      const result = formatLinkHeader([{href: '/path%20here', rel: 'next'}]);
      assert.equal(result, '</path%20here>; rel="next"');
    });

    it('accepts href with query containing reserved characters', () => {
      const result = formatLinkHeader([{href: '/items?page=1&sort=date', rel: 'next'}]);
      assert.equal(result, '</items?page=1&sort=date>; rel="next"');
    });

    it('accepts href with fragment', () => {
      const result = formatLinkHeader([{href: '/items#section', rel: 'next'}]);
      assert.equal(result, '</items#section>; rel="next"');
    });
  ...
  describe('paginationLinks', () => {
    ...
    it('clamps prev to lastPage when page far exceeds lastPage', () => {
      const links = paginationLinks({baseUrl: '/items', page: 10, perPage: 10, total: 50});
      const rels = links.map(l => l.rel);
      assert.deepEqual(rels, ['first', 'prev', 'last']);
      const prev = links.find(l => l.rel === 'prev');
      assert.ok(
        prev.href.includes('page=5'),
        'prev should point to lastPage (5), not page - 1 (9)'
      );
    });

    it('clamps prev to lastPage when page is lastPage + 1', () => {
      const links = paginationLinks({baseUrl: '/items', page: 6, perPage: 10, total: 50});
      const rels = links.map(l => l.rel);
      assert.deepEqual(rels, ['first', 'prev', 'last']);
      const prev = links.find(l => l.rel === 'prev');
      assert.ok(prev.href.includes('page=5'), 'prev should point to lastPage (5)');
    });

    it('suppresses next when page exceeds lastPage', () => {
      const links = paginationLinks({baseUrl: '/items', page: 10, perPage: 10, total: 50});
      const rels = links.map(l => l.rel);
      assert.ok(!rels.includes('next'), 'next should not appear when page > lastPage');
    });
