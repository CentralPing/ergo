...
/** RFC 3986 §2 URI-reference character repertoire: unreserved, reserved, pct-encoded. */
const URI_REF_CHARS_RE = /^[A-Za-z0-9\-._~:/?#[\]@!$&'()*+,;=%]+$/;
 ...
 * @throws {TypeError} If `href` contains characters not permitted in a URI-reference (RFC 3986 §2) or a parameter key is not a valid token
 */
export function formatLinkHeader(links) {
  return links
    .map(({href, rel, ...params}) => {
      if (!URI_REF_CHARS_RE.test(String(href))) {
 ...
 *
 * Only includes `prev` when `page > 1` (clamped to `lastPage` when `page`
 * exceeds the last page) and `next` when `page < lastPage`. `first` and
 * `last` are always included.
 *
