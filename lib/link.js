 ...
 * @throws {TypeError} If `href` contains forbidden characters (CR, LF, NUL, or `>`) or a parameter key is not a valid token
 */
export function formatLinkHeader(links) {
  return links
    .map(({href, rel, ...params}) => {
      if (/[\r\n\0>]/.test(String(href))) {
        throw new TypeError('Link href contains forbidden characters');
      }
