 ...
 * // => {tag: ['a', 'b']}
 */
import {trySet} from '../utils/set.js';

const subpropRegExp = /^([^[]+)(?:\[(.*)\]|)$/;

/** Strict non-negative integer string — numeric bracket indices under Arrays. */
...
/**
 * True when a nested write would attach a non-index property under an existing Array
 * (empty-bracket first-wins). Numeric indices under Arrays remain allowed (#381).
 * @param {object} obj - Accumulator
 * @param {string} path - Dot-delimited destination path
 * @returns {boolean} - True when nesting a non-index segment under an Array prefix
 */
function nestsNonIndexUnderArray(obj, path) {
  ...
  for (let i = 0; i < segments.length - 1; i++) {
    ...
    cur = cur[segments[i]];
    if (Array.isArray(cur) && !IS_ARRAY_INDEX.test(segments[i + 1])) {
      return true;
    }
  }
  return false;
...
export default (query, {split = true, maxPairs = 256, maxLength = 8192} = {}) => {
  ...
  for (const [k, v] of Object.entries(q)) {
    ...
    // Bidirectional first-wins for scalar vs nested conflicts (#354, #379, #380, #381):
    ...
    if (nestsNonIndexUnderArray(acc, prop)) {
      continue;
    }
