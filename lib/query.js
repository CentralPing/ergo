...
export default (query, {split = true, maxPairs = 256, maxLength = 8192} = {}) => {
  ...
  for (const [k, v] of Object.entries(q)) {
    ...
    // Bidirectional first-wins for scalar vs nested conflicts (#354, #379):
    // - a=42&a[b]=99 → trySet throws ERGO_SET_PATH_TRAVERSE → skip nested
    // - a[b]=99&a=42 → skip bare overwrite of an existing container
    const dotIdx = prop.indexOf('.');
    if (dotIdx === -1 && Object.hasOwn(acc, prop)) {
      const existing = acc[prop];
