...
/**
 * True when a nested assignment would write under an existing Array (empty-bracket first-wins).
 * @param {object} obj - Accumulator
 * @param {string} path - Dot-delimited destination path
 * @returns {boolean} - True when any proper path prefix is an Array
 */
function nestsUnderArray(obj, path) {
...
export default (query, {split = true, maxPairs = 256, maxLength = 8192} = {}) => {
  ...
  for (const [k, v] of Object.entries(q)) {
    ...
    // Bidirectional first-wins for scalar vs nested conflicts (#354, #379, #380):
    ...
    // - a[]=2&a[b]=1 → skip nesting under an Array established by empty brackets
    if (isContainer(getOwnAtPath(acc, prop)) || nestsUnderArray(acc, prop)) {
      continue;
    }
