...
describe('[Boundary] lib/redact-headers', () => {
  ...
  it('matching is case-insensitive (RFC 9110)', () => {
    const headers = {Authorization: 'Bearer token', 'content-type': 'text/html'};
    const result = redactHeaders(headers, DEFAULT_REDACTED_HEADERS);
    assert.equal(result.Authorization, '[REDACTED]');
  ...
  it('redacts when custom set entries use non-lowercase names', () => {
    const headers = {'set-cookie': 'session=abc', 'x-custom': 'safe'};
    const mixedCaseSet = new Set(['Set-Cookie', 'X-Custom']);
    const result = redactHeaders(headers, mixedCaseSet);
    assert.equal(result['set-cookie'], '[REDACTED]');
    assert.equal(result['x-custom'], '[REDACTED]');
  });
});
