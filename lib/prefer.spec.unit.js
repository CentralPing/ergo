...
describe('[Boundary] lib/prefer', () => {
  ...
  it('skips trailing text after malformed quoted value without creating orphan keys', () => {
    const result = parsePrefer('bad="a\x00"orphan, return=minimal');
  ...
  it('recovers after unterminated quoted value without swallowing later preferences', () => {
    const result = parsePrefer('respond-async; p="unterminated, return=minimal');
    assert.equal(result['respond-async'], true);
    assert.equal(result.return, 'minimal');
  });

  it('recovers after stray quote in unquoted context', () => {
    const result = parsePrefer('foo=bar"baz, qux=ok');
    assert.equal(result.foo, undefined);
    assert.equal(result.qux, 'ok');
  });

  it('normalizes mixed-case name and value (Return=Minimal)', () => {
    assert.deepEqual(parsePrefer('Return=Minimal'), np({return: 'minimal'}));
  });

  it('normalizes uppercase bare token (RESPOND-ASYNC)', () => {
    assert.deepEqual(parsePrefer('RESPOND-ASYNC'), np({'respond-async': true}));
  });

  it('normalizes mixed-case value (Handling=STRICT)', () => {
    assert.deepEqual(parsePrefer('Handling=STRICT'), np({handling: 'strict'}));
  });

  it('normalizes quoted value casing (FOO="Bar Baz")', () => {
    assert.deepEqual(parsePrefer('FOO="Bar Baz"'), np({foo: 'bar baz'}));
  });

  it('normalizes multiple mixed-case preferences', () => {
    const result = parsePrefer('Return=Minimal, HANDLING=Lenient');
    assert.deepEqual(result, np({return: 'minimal', handling: 'lenient'}));
  });
});
