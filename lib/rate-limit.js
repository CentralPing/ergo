...
export class MemoryStore {
   ...
   * @returns {{count: number, resetMs: number, resetAt: number}} - Current count, ms until the
   *   oldest entry expires, and absolute reset time (ms) in this store's clock domain
   */
  hit(key, windowMs) {
    const now = this._now();
    ...
    return {count: timestamps.length, resetMs, resetAt};
  }
 ...
 *
 * @param {object} store - Store implementing `hit(key, windowMs) => {count, resetMs, resetAt}`
 * @param {string} key - Client identifier
 ...
 *   - `reset`: Unix timestamp (seconds) when the window resets (from store `resetAt`)
 *   - `limited`: true when count exceeds max
 *   - `retryAfter`: seconds until retry is allowed (only when limited)
 * @throws {TypeError} If store `hit()` returns a non-finite `resetAt` or `resetMs`
 */
export function checkRateLimit(store, key, max, windowMs) {
  const {count, resetMs, resetAt} = store.hit(key, windowMs);

  if (!Number.isFinite(resetAt)) {
    throw new TypeError('resetAt must be a finite number');
  }
  if (!Number.isFinite(resetMs)) {
    throw new TypeError('resetMs must be a finite number');
  }
