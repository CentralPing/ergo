...
describe('[Boundary] lib/cookie/cookie', () => {
  ...
  describe('boxed string and custom toString() coercion', () => {
    it('rejects boxed string value with injection payload', () => {
      const cookie = bake('x', 'y');

      cookie.value = new String('ok; Domain=evil.com');
      assert.throws(() => cookie.toHeader(), TypeError);
    });

    it('rejects boxed string name with injection payload', () => {
      const cookie = bake('x', 'y');

      cookie.name = new String('bad name');
      assert.throws(() => cookie.toHeader(), TypeError);
    });

    it('rejects object with custom toString() in value', () => {
      const cookie = bake('x', 'y');
      cookie.value = {toString: () => 'ok; Path=/evil'};
      assert.throws(() => cookie.toHeader(), TypeError);
    });

    it('rejects object with custom toString() in domain', () => {
      const cookie = bake('x', 'y');
      cookie.domain = {toString: () => 'evil; Path=/'};
      assert.throws(() => cookie.toHeader(), TypeError);
    });

    it('rejects object with custom toString() in path', () => {
      const cookie = bake('x', 'y');
      cookie.path = {toString: () => '/ok; Domain=evil.com'};
      assert.throws(() => cookie.toHeader(), TypeError);
    });

    it('rejects object with custom toString() in sameSite', () => {
      const cookie = bake('x', 'y');
      cookie.sameSite = {toString: () => 'Lax; Domain=evil.com'};
      assert.throws(() => cookie.toHeader(), TypeError);
    });

    it('coerces valid boxed string value without rejection', () => {
      const cookie = bake('x', 'y');

      cookie.value = new String('safe');
      assert.ok(cookie.toHeader().includes('x=safe'));
    });

    it('coerces numeric value to string', () => {
      const cookie = bake('x', 'y');
      cookie.value = 42;
      assert.ok(cookie.toHeader().includes('x=42'));
    });
  });
});
