...
function toHeader({
  ...
  httpOnly,
  maxAge = rawValue === undefined ? 0 : undefined,
  expires = rawValue === undefined ? Date.now() + maxAge : undefined
} = {}) {
  const name = rawName !== undefined ? String(rawName) : undefined;
  const value = rawValue !== undefined ? String(rawValue) : undefined;
  const domain = rawDomain !== undefined ? String(rawDomain) : undefined;
  const path = rawPath !== undefined ? String(rawPath) : undefined;
  const sameSite = rawSameSite !== undefined ? String(rawSameSite) : undefined;
