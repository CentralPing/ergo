...
describe('[Boundary] lib/idempotency', () => {
  ...
  describe('IdempotencyStore _completeKeys side index', () => {
    let store;

    beforeEach(() => {
      store = new IdempotencyStore({maxKeys: 5, ttlMs: 1000});
    });

    it('side index remains consistent after complete then delete then re-set', () => {
      const gen = store.set('k1', 'fp');
      store.complete('k1', {statusCode: 200}, gen);
      assert.equal(store._completeKeys.size, 1);

      store.delete('k1');
      assert.equal(store._completeKeys.size, 0);

      store.set('k1', 'fp-new');
      assert.equal(store._completeKeys.size, 0);
      assert.equal(store.get('k1').status, 'processing');
    });

    it('TTL prune removes completed keys from side index', async () => {
      store = new IdempotencyStore({maxKeys: 5, ttlMs: 10});
      const gen1 = store.set('k1', 'fp');
      const gen2 = store.set('k2', 'fp');
      store.complete('k1', {statusCode: 200}, gen1);
      store.complete('k2', {statusCode: 200}, gen2);
      assert.equal(store._completeKeys.size, 2);
    ...
    it('re-setting a complete key removes it from side index before re-insertion as processing', () => {
      const gen = store.set('k1', 'fp');
      store.complete('k1', {statusCode: 200}, gen);
      assert.equal(store._completeKeys.size, 1);
      assert.ok(store._completeKeys.has('k1'));
    ...
    it('eviction via side index selects from _completeKeys (O(1) lookup)', () => {
      ...
      store.complete('c', {statusCode: 200}, genC);
      assert.ok(store._completeKeys.has('c'));
    ...
    it('eviction selects the first-completed entry (Set insertion order)', () => {
      store = new IdempotencyStore({maxKeys: 4});
      store.set('a', 'fp');
      const genB = store.set('b', 'fp');
      const genC = store.set('c', 'fp');
    ...
    it('when _completeKeys is empty, falls back to oldest-entry eviction', () => {
      const warnings = [];
      const orig = process.emitWarning;
      process.emitWarning = (msg, opts) => warnings.push({message: msg, ...opts});
    ...
    it('invariant: _completeKeys.size equals count of complete entries', () => {
      const gen1 = store.set('k1', 'fp');
      const gen2 = store.set('k2', 'fp');
      const gen3 = store.set('k3', 'fp');
      ...
      store.complete('k2', {statusCode: 201}, gen2);
      assert.equal(store._completeKeys.size, 2);

      store.delete('k1');
      assert.equal(store._completeKeys.size, 1);

      store.complete('k3', {statusCode: 202}, gen3);
      assert.equal(store._completeKeys.size, 2);

      store.set('k2', 'fp-new');
      assert.equal(store._completeKeys.size, 1);
      assert.ok(store._completeKeys.has('k3'));
    });

    it('get() lazy expiry removes expired complete key from side index', async () => {
      store = new IdempotencyStore({maxKeys: 5, ttlMs: 10});
      const gen = store.set('k1', 'fp');
      store.complete('k1', {statusCode: 200}, gen);
      assert.equal(store._completeKeys.size, 1);
