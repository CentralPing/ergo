...
describe('[Boundary] lib/response-time', () => {
  ...
  describe('resolveTimingConfig', () => {
    it('returns undefined when timing is undefined', () => {
      assert.equal(resolveTimingConfig(undefined), undefined);
    });

    it('returns undefined when timing is false', () => {
      assert.equal(resolveTimingConfig(false), undefined);
    });

    it('returns undefined when timing is null', () => {
      assert.equal(resolveTimingConfig(null), undefined);
    });

    it('returns defaults when timing is true', () => {
      assert.deepEqual(resolveTimingConfig(true), {
        header: DEFAULT_TIMING_HEADER,
        precision: DEFAULT_TIMING_PRECISION
      });
    });

    it('returns defaults when timing is an empty object', () => {
      assert.deepEqual(resolveTimingConfig({}), {
        header: DEFAULT_TIMING_HEADER,
        precision: DEFAULT_TIMING_PRECISION
      });
    });

    it('applies custom header with default precision', () => {
      assert.deepEqual(resolveTimingConfig({header: 'server-timing'}), {
        header: 'server-timing',
        precision: DEFAULT_TIMING_PRECISION
      });
    });

    it('applies custom precision with default header', () => {
      assert.deepEqual(resolveTimingConfig({precision: 0}), {
        header: DEFAULT_TIMING_HEADER,
        precision: 0
      });
    });

    it('applies both custom header and precision', () => {
      assert.deepEqual(resolveTimingConfig({header: 'x-time', precision: 2}), {
        header: 'x-time',
        precision: 2
      });
    });

    it('preserves precision 0 (does not fall through via ??)', () => {
      const config = resolveTimingConfig({precision: 0});
      assert.equal(config.precision, 0);
      assert.notEqual(config.precision, DEFAULT_TIMING_PRECISION);
    });
  });
