...
/** Entry lifecycle status while a request is in flight. @type {string} */
export const STATUS_PROCESSING = 'processing';

/** Entry lifecycle status after a response has been stored for replay. @type {string} */
export const STATUS_COMPLETE = 'complete';

/**
 * In-memory idempotency key store with TTL-based expiry.
 *
 * Entries transition through states: {@link STATUS_PROCESSING} -> {@link STATUS_COMPLETE}.
 * Expired entries are pruned lazily on `get()` calls and eagerly
 * in `set()` before capacity checks. Eviction prefers {@link STATUS_COMPLETE}
 * entries over {@link STATUS_PROCESSING} entries to protect in-flight requests.
...
export class IdempotencyStore {
   ...
   * @returns {Readonly<{fingerprint: string, response: (object | undefined), status: ('processing' | 'complete'), expiresAt: number}> | undefined} - Frozen entry snapshot, or undefined
   */
  get(key) {
  ...
  /**
   * Store a new idempotency entry in {@link STATUS_PROCESSING} state.
   * @param {string} key - Idempotency key
  ...
  /**
   * Mark an entry as {@link STATUS_COMPLETE} with the response to replay. The response is
