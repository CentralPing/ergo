...
export class IdempotencyStore {
  ...
  /**
   * Retrieve a stored entry by key. Returns a frozen snapshot of the stored
   * entry — mutations do not affect store state. Returns `undefined` if
   * not found or expired.
   * @param {string} key - Idempotency key
   * @returns {Readonly<{fingerprint: string, response: object, status: string, expiresAt: number}> | undefined} - Frozen entry snapshot, or undefined
   */
  get(key) {
    ...
    snapshot.response = entry.response
      ? Object.freeze({
          ...entry.response,
          ...(entry.response.headers ? {headers: [...entry.response.headers]} : undefined)
        })
      : undefined;
   ...
   * stored as a shallow copy — the caller may safely mutate the original
  ...
  complete(key, response) {
    ...
    if (entry) {
      const clonedResponse = {
        ...response,
        ...(response.headers ? {headers: [...response.headers]} : undefined)
      };
