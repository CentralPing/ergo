...
const WILDCARD_CREDENTIALS_CODE = 'ERGO_CORS_WILDCARD_CREDENTIALS';
...
export default ({
  ...
  if (origins === '*' && allowCredentials) {
    if (!emittedWarnings.has(WILDCARD_CREDENTIALS_CODE)) {
      emittedWarnings.add(WILDCARD_CREDENTIALS_CODE);
      process.emitWarning(
        "cors() configured with origins: '*' and allowCredentials: true — any site can make " +
          'credentialed cross-origin requests. Use an explicit origin allowlist for production.',
        {type: 'ErgoWarning', code: WILDCARD_CREDENTIALS_CODE}
      );
    }
  }
  ...
  const preflightHeaders = flatArray(
    ...
    {h: 'Vary', v: 'Access-Control-Request-Method'},
  ...
  /** @param {{ origin?: string, method?: string, requestMethod?: string, requestHeaders?: string[] }} [req] - CORS request properties */
  const validator = ({origin, method, requestMethod, requestHeaders} = {}) => {
