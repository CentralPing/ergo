...
describe('[Module] lib/body/multiparse', () => {
  ...
  it('returns zero parts when maxParts is 0', () => {
    const raw = buildMultipart([{name: 'f', body: 'v'}], boundary);
    const parts = multiparse(raw, boundary, {maxParts: 0});
    assert.equal(parts.length, 0);
  });

  it('returns all parts when count equals maxParts', () => {
    const fields = Array.from({length: 3}, (_, i) => ({name: `f${i}`, body: `v${i}`}));
    const raw = buildMultipart(fields, boundary);
    const parts = multiparse(raw, boundary, {maxParts: 3});
    assert.equal(parts.length, 3);
  });
