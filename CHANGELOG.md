...
- **`formatLinkHeader` href validation replaced with RFC 3986 URI-reference character
  ...
  unreserved, reserved, and percent-encoding characters are accepted. Callers passing
  href values with spaces, `<`, `{`, `}`, `\`, `^`, `` ` ``, or `|` will now receive
  a `TypeError`. Callers producing IRIs must percent-encode non-ASCII characters before
  calling `formatLinkHeader`.
